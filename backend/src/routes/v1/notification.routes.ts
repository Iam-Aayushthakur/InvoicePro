...
export async function handleNotificationRoutes(request: Request, pathSegments: string[], env: Env): Promise<Response> {
  try {
    ...
    if (method === 'GET' && seg1 === '') return NotificationController.list(request, user, env);
    if (method === 'POST' && seg1 === '') return NotificationController.create(request, user, env);
    if (method === 'POST' && seg1 === 'mark-all-read') return NotificationController.markAllRead(request, user, env);
    if (method === 'PATCH' && seg1 && seg2 === 'read') return NotificationController.markRead(request, seg1, user, env);

    return errorResponse('Notification route not found', 404, 'NOT_FOUND');
  } catch (err) {
