import { ReportingController } from '../../controllers/reporting/index.js';
...
export async function handleReportingRoutes(request: Request, pathSegments: string[], env: Env): Promise<Response> {
  try {
    const session = await authenticateRequest(request, env);
    const user = await resolveTenantContext(session.authUserId, request.headers.get('x-company-id'), env);
    const method = request.method;

    if (method === 'GET') return ReportingController.generate(request, user, env);

    return errorResponse('Reporting route not found', 404, 'NOT_FOUND');
  } catch (err: any) {
