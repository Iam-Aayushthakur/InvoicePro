...
import { useCreateSupplier, useUpdateSupplier, Supplier } from '../../../hooks/api/useSuppliers';
...
export function SupplierForm({ isOpen, onClose, supplier }: SupplierFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (supplier) {
      ...
      createSupplier.mutate(formData, { onSuccess: () => onClose() });
    }
  };
    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 w-full sm:w-96 bg-white shadow-2xl z-50 flex flex-col animate-in slide-in-from-right duration-300">
        <div className="flex items-center justify-between p-6 border-b border-slate-100">
          <h2 className="text-lg font-bold text-slate-900">{supplier ? 'Edit Supplier' : 'Add New Supplier'}</h2>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="space-y-2">
            ...
            <input required name="name" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="flex h-11 w-full rounded-lg border border-slate-300 px-3 text-sm focus:ring-2 focus:ring-indigo-600 outline-none" placeholder="Acme Suppliers" />
          </div>

          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-700">Email Address</label>
            <input type="email" name="email" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} className="flex h-11 w-full rounded-lg border border-slate-300 px-3 text-sm focus:ring-2 focus:ring-indigo-600 outline-none" placeholder="vendor@acme.com" />
          </div>
