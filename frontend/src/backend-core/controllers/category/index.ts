...
import { validateCreateCategory, validateUpdateCategory } from '../../core/validators/category.validator.js';
...
export const CategoryController = {
  async list(request: Request, user: UserContext, env: Env): Promise<Response> {
    try {
      ...
      const { data, total } = await CategoryService.listCategories(user, page, limit, search, env);
      return successResponse({ categories: data, total, page, limit, totalPages: Math.ceil(total / limit) });
    } catch (e) { return handleErr(e); }
  },
  async get(id: string, user: UserContext, env: Env): Promise<Response> {
    try { return successResponse({ category: await CategoryService.getCategory(id, user, env) }); }
    catch (e) { return handleErr(e); }
  },
  async create(request: Request, user: UserContext, env: Env): Promise<Response> {
    try {
      const body = await request.json();
      const val = validateCreateCategory(body);
      if (!val.success) return errorResponse('Validation failed', 400, 'VALIDATION_ERROR', val.errors);
      return successResponse({ category: await CategoryService.createCategory(val.data!, user, env) }, 201);
    } catch (e) { return handleErr(e); }
  },
  async update(request: Request, id: string, user: UserContext, env: Env): Promise<Response> {
    try {
      const body = await request.json();
      const val = validateUpdateCategory(body);
      if (!val.success) return errorResponse('Validation failed', 400, 'VALIDATION_ERROR', val.errors);
      return successResponse({ category: await CategoryService.updateCategory(id, val.data!, user, env) });
    } catch (e) { return handleErr(e); }
  },
  async deactivate(id: string, user: UserContext, env: Env): Promise<Response> {
    try {
      await CategoryService.deactivateCategory(id, user, env);
