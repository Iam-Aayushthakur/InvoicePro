import { AuditController } from '../../controllers/audit/index.js';
...
export async function handleAuditRoutes(request: Request, pathSegments: string[], env: Env): Promise<Response> {
  try {
    const session = await authenticateRequest(request, env);
    const user = await resolveTenantContext(session.authUserId, request.headers.get('x-company-id'), env);
    const method = request.method;

    if (method === 'GET') return AuditController.list(request, user, env);

    return errorResponse('Audit route not found', 404, 'NOT_FOUND');
  } catch (err: any) {
