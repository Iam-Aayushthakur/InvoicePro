...
export async function handleInventoryRoutes(request: Request, pathSegments: string[], env: Env): Promise<Response> {
  try {
    ...
    if (method === 'GET' && seg1 === '') return InventoryController.list(request, user, env);
    if (method === 'POST' && seg1 === 'adjust') return InventoryController.adjust(request, user, env);
    if (method === 'GET' && seg1 && seg2 === 'transactions') return InventoryController.transactions(request, seg1, user, env);
    if (method === 'GET' && seg1) return InventoryController.get(seg1, user, env);

    return errorResponse('Inventory route not found', 404, 'NOT_FOUND');
  } catch (err) {
