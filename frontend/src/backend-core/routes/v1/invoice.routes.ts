...
export async function handleInvoiceRoutes(request: Request, pathSegments: string[], env: Env): Promise<Response> {
  try {
    ...
    if (method === 'GET' && seg1 === '') return InvoiceController.list(request, user, env);
    if (method === 'POST' && seg1 === '') return InvoiceController.create(request, user, env);
    if (method === 'GET' && seg1) return InvoiceController.get(seg1, user, env);
    if (method === 'PATCH' && seg1 && seg2 === 'status') return InvoiceController.updateStatus(request, seg1, user, env);

    return errorResponse('Invoice route not found', 404, 'NOT_FOUND');
  } catch (err) {
